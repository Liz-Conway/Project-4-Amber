...
class AddUser(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    """
    View containing the logic for the Admin to add new users.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if add_user_form.is_valid():
            ...
            messages.success(
                request, 
                f'New User <span class="name">{first_name} {last_name}\
...
class MyAccount(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if me.is_anonymous:
            messages.warning(
                request, 
                'User is not logged in.  Please use the menu to log in.')
            return render(
                request,
                'index.html',
                {}
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Gets all of the data that we posted from our form
        user = request.user
        my_account_form = HippotherapyUserChangeForm(
        ...
        if my_account_form.is_valid():
            ...
            messages.success(
                request, f'You updated your account details successfully.',
                             extra_tags='safe')
        else:
            # If the form is NOT valid  
            # then return an  empty 'add user' form instance
