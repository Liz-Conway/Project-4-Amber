...
class AddClient(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if add_user_form.is_valid():
            ...
            messages.success(
                request,
                f'New client <span class="name">{first_name} {last_name}\
                    </span> added successfully.', extra_tags='safe')
        else:
            ...
            messages.error(
                request,
                '<span class="boldEntry">Invalid form submission.</span>',
                extra_tags='safe')
...
class EditClient(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        client_diagnoses = Client.objects.filter(
            id=client_id).values('diagnosis')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if edit_client_form.is_valid():
            ...
            update_diagnoses(edited_client.id, request.POST)
            new_hat = get_object_or_404(
                Hat.objects.filter(id=request.POST['hat_size']))
            ...
            messages.success(
                request,
                f'Client <span class="name">{first_name} {last_name}\
                    </span> has been changed successfully', extra_tags='safe')
...
class RecordSession(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        max_sessions = Session.objects.filter(
            course=course).order_by('-week_number')
        ...
        # If a new session has been created
        # then we got here from a breadcrumb link
        if int(last_session) != max_session_id:
            ...
            session = Session(
                course=course, session_date=session_date,
                week_number=session_week)
    ...
    def post(self, request, client, *args, **kwargs):
        ...
        # Save the Record Session data
        # in case the user clicks the 'Record Session' breadcrumb link
        ...
        if record_session_form.is_valid():
            ...
            old_sessions = Session.objects.filter(
                course=course_number).order_by('-week_number')
            if old_sessions.count() != 0:
                ...
                if int(
                    week_number) == duplicate_session.week_number and int(
                        course_number) == duplicate_session.course.id:
            ...
            messages.success(
                request,
                f'New session for <span class="boldEntry">{session_client}\
                </span> ({course_number}/{week_number}) added successfully.',
                extra_tags='safe')

            # Redirect to the 'observeSession' page after adding a new Session
            # Works by going to the URL paths defined in 'urls.py'
            # Finds the method whose 3rd ('name=') parameter is the
            # String passed into redirect
...
class SelectClient(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if page_url == 'generateChart':
            ...
            if courses.count() != 0:
                ...
                max_session_dates = Session.objects.filter(
                    course__in=courses).values('course').annotate(
                        max_date=Max('session_date')).order_by(
                            '-max_date', '-course')
                ...
                sesh = Session.objects.filter(
                    session_date=last_session_date['max_date'],
                    course=last_session_date['course']).order_by(
                        '-week_number')
                # Get the last session of the last Course
                # taken by this client
                last_sesh = sesh[0]
                # Get the id of the last session of the
                # last Course taken by this client
    ...
    def test_func(self):
        return self.request.user.user_role(
            ) == 'Occupational Therapist' or self.request.user.user_role(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(observation_data) == number_of_skills + 2:
            for observation in observation_data:
                ...
                # observation will be of the form 'scoreX' or
                # scoreXX' where X is a digit
                # Splitting the string leaves a blank character
                # as the first element
                # of the returned array, so take the element
                # at index 1 and convert to an int
                ...
                # Get the session object for the session ID passed into the
                # post method
                session_query = Session.objects.filter(id=session_id)
                session = get_object_or_404(session_query)

                # Create a SkillScore object for this Skill,
                # during this Session
                # having the observed score
                skill_score = SkillScore(
                    session=session, skill=skill, score=score)
                # Save the SkillScore object to the 'through' table
                skill_score.save()

            course = get_object_or_404(
                Course.objects.filter(courses__id=session_id))
            course_id = course.id
            ...
            messages.error(
                request,
                'You need to add a score on <span class="boldEntry">\
                every</span> skill in <span class="boldEntry">\
                all</span> functions', extra_tags='safe')
            return HttpResponseRedirect(
                reverse('observeSession', args=[session_id, last_session]))
...
class ChooseSession(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        sessions = Session.objects.filter(
            course__client=client).exclude(score_session=None)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages.error(
                request,
                "You need to select one of the sessions \
                before clicking the button")
            return HttpResponseRedirect(
                reverse('chooseSession', args=[client_id]))

        return HttpResponseRedirect(
            reverse('viewSession', args=[session_id, client_id]))
...
class ViewSession(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        tasks = Task.objects.filter(
            performed__id=session_id).order_by('-mounted')
...
class ChartPage(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    """
    We will be showing the current score and the
    baseline score for each function
    in a chart.
    """
    template_name = "hippo/generateChart.html"

    # In class-based views:
    # Instead of using an if statement to check
    # the request method,
    # we simply create class methods called GET,
    # POST, or any other HTTP verb.

    # We need to retrieve Labels, current scores and
    #  baseline scores from the database.
    # We also need to retrieve and calculate the
    # maximum score for each Function.
    # We then need to convert the actual scores to
    #  scores as a percentage of the total.
    # We need to pass Labels, current scores and
    # baseline scores to the chart page.
    def get(self, request, *args, **kwargs):
        # '*args' = Standard arguments parameter
        # '**kwargs' = Standard keyword arguments parameter
        ...
        client = get_object_or_404(
            Client.objects.filter(participates__id=course_id))

        # We need to find the latest week for a given course that has scores
        latest_week = Session.objects.filter(
            course=course_id).exclude(score_session=None).aggregate(
        ...
        last_session = Session.objects.filter(
            course=course_id, week_number=latest_week)[0]
        latest_session = last_session.id
        # Find the scores for the last scoring week of a given course
        # Need to have .values() before .annotate()
        # in order to perform GROUP BY
        scores_query = SkillScore.objects.values(
            'skill__function__id').filter(
                session=latest_session).annotate(
                    Sum('score')).order_by('skill__function_id')
        scores_list = [score_query.get(
            'score__sum') for score_query in scores_query]

        # Find the number of skills in each Function
        total_query = Skill.objects.values(
            'function__function_name').annotate(
                Count('function_id')).order_by('function_id')
        # Get the labels for the chart
        functions = json.dumps(
            [total['function__function_name'] for total in total_query])
        ...
        percent_scores = [round(
            score * 100 / total) for score, total in
         zip(scores_list, totals)]
        # Format the percent scores to be sent to the chart
        scores = json.dumps(percent_scores)

        # We need to find the first week for a given course
        # that has scores, in order to calculate a baseline
        first_week = Session.objects.filter(
            course=course_id).exclude(score_session=None).aggregate(
                Min('week_number'))['week_number__min']
        # Find the session for the first scoring week of a given course
        first_session = Session.objects.filter(
            course=course_id, week_number=first_week).values('id')[0]['id']
        # Find the scores for the last scoring week of a given course
        # Need to have .values() before .annotate() in
        # order to perform GROUP BY
        baselines_query = SkillScore.objects.values(
            'skill__function__id').filter(
                session=first_session).annotate(
                    Sum('score')).order_by('skill__function_id')
        baseline_list = [baseline_query.get(
            'score__sum') for baseline_query in baselines_query]
        # Calculate each baseline score as a percentage of its total
        percent_baselines = [round(
            score * 100 / total) for score, total in
           zip(baseline_list, totals)]
...
class ChooseCourse(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages.error(
                request, 'You must select one of the courses below')
            return HttpResponseRedirect(
                reverse('chooseCourse', args=[client_id]))

        return HttpResponseRedirect(
            reverse('generateChart', args=[course_id]))
...
class DeleteClient(DeleteView):
    ...
    Inherits from Djangos DeleteView class where the magic of deletion occurs.
    Requires a 'client_confirm_delete.html' template, which is displayed,
    only on confirmation on that page is the Client
    ...
    def get_success_url(self):
        ...
        messages.success(
            self.request,
            f'<span class="name">{client_name}</span> was successfully \
            deleted from Amber', extra_tags='safe')
        return '/getClients'
