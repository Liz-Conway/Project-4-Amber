...
from hippotherapy.models import Session, Skill, SkillScore, Course, Hat,\
    Function
from hippotherapy.models import Client as HipClient
...
class ViewsTest(TestCase):
    def setUp(self):
        self.hat = Hat.objects.create(size='4')
        self.hip_client = HipClient.objects.create(
            first_name = 'Mister',
            last_name = 'Blobby',
            gender = 'M',
        ...
        self.choose_course_client_session = reverse('chooseCourse', args=[self.hip_client.id, self.hip_session.id])
    ...
    def test_get_add_client(self):
        response = self.client.get(self.add_client_url)
    ...
    def test_get_record_session(self):
        response = self.client.get(self.record_session_url)
    ...
    def test_get_record_session_no_client(self):
        response = self.client.get(reverse('recordSession', args=[666]))
    ...
    def test_get_select_client(self):
        response = self.client.get(self.select_client_url)
    ...
    def test_get_select_client_any_target(self):
        response = self.client.get(reverse('selectClient', args=['any target']))
    ...
    def test_get_observe_session(self):
        response = self.client.get(self.observe_session_url)
    ...
    def test_get_observe_session_no_session(self):
        response = self.client.get(reverse('observeSession', args=[666]))
    ...
    def test_get_generate_chart(self):
        ...
        response = self.client.get(self.generate_chart_url)
    ...
    def test_get_generate_chart_no_course(self):
        # Needed to add some skill scores for this to work
        response = self.client.get(reverse('generateChart', args=[666]))
    ...
    def test_get_choose_course_client_only(self):
        response = self.client.get(self.choose_course_client)
    ...
    def test_get_choose_course_no_client(self):
        response = self.client.get(reverse('chooseCourse', args=[666]))
    ...
    def test_get_choose_course_client_session(self):
        response = self.client.get(self.choose_course_client_session)
    
        self.assertEqual(response.status_code, 200, "Going to Choose Course page should return a Status Code of 200 (OK)")
        self.assertTemplateUsed(response, "hippo/chooseCourse.html", "Choose Course page should use the chooseCourse.html template")

    def test_get_choose_course_no_session(self):
        response = self.client.get(reverse('chooseCourse', args=[self.hip_client.id, 666]))
    
        self.assertEqual(response.status_code, 404, "Going to Choose Course page with a non-existant session should return a Status Code of 404")

    def test_get_new_course(self):
        response = self.client.get(self.new_course)
