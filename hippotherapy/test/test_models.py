...
class SkillScoreTest(TestCase):
    ...
    def setUp(self):
        ...
        client = Client.objects.create(first_name='Sir', 
        last_name='Test', gender='M', date_of_birth=test_date, hat_size=hat)
        course = Course.objects.create(client=client)
        horse = Horse.objects.create(horse_name='Red Rum')
        self.session = Session.objects.create(course=course, 
                                              week_number=1, 
                                              session_date=test_date, 
                                              horse=horse)
        self.session2 = Session.objects.create(course=course, 
                                               week_number=2, 
                                               session_date=test_date, 
                                               horse=horse)
        ...
        SkillScore.objects.create(session=self.session, 
                                  skill=self.basket_weaving, score=1)
        SkillScore.objects.create(session=self.session, 
                                  skill=self.flower_arranging, score=2)
        SkillScore.objects.create(session=self.session, 
                                  skill=self.embroidery, score=5)
        # Scores for session 2
        SkillScore.objects.create(session=self.session2, 
                                  skill=self.basket_weaving, score=3)
        SkillScore.objects.create(session=self.session2,
                                   skill=self.flower_arranging, score=4)
        SkillScore.objects.create(session=self.session2, 
                                  skill=self.embroidery, score=2)
    ...
    def test_skill_score_is_1(self):
        ...
        self.assertEqual(1, score_1.count(), 
                         'There should only be one Skill \
                         that has a score of 1')
        # Check the name of the Skill with a score of 1
        self.assertEqual('Basket Weaving', 
                         score_1[0].skill_name, 
                         'The only Skill with score of 1 should be Basket Weaving')
    ...
    def test_skill_score_is_2(self):
        ...
        self.assertEqual(2, score_2.count(), 
                         'There should two Skills that have a score of 2')
        # Check the name of the Skill with a score of 2
        self.assertEqual('Flower Arranging', 
                         score_2[0].skill_name, 
                         'The only Skill with a score of 2\
                          should be Flower Arranging')
