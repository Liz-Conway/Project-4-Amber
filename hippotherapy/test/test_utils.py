...
class UtilTest(TestCase):
    ...
    def setUp(self):
        ...
        self.task1 = Task.objects.create(task_name='Dance merengue', 
                                         mounted=True)
        self.task2 = Task.objects.create(task_name='Pole vault',
                                          mounted=False)
    ...
    def test_get_course_no_sessions(self):
        ...
        self.assertEqual(
            course, 
            self.course_no_session, 
            "Should return a course that has no sessions")
    ...
    def test_get_course_no_sessions_has_sessions(self):
        ...
        self.assertEqual(
            course, 
            None, 
            "Should return NONE when no courses have no sessions")
    ...
    def test_get_course_for_client(self):
        ...
        self.assertEqual(
            course, 
            self.course, 
            "Should return a course for Mister Blobby")
    ...
    def test_get_course_for_client_2_courses(self):
        ...
        self.assertEqual(
            course, 
            second_course, 
            "Should return the latest course for Mister Blobby")
    ...
    def test_get_course_for_client_no_courses(self):
        ...
        self.assertNotEqual(
            course, 
            None, 
            "Courseless client should be given a new course")
        self.assertNotEqual(
            course.id, 
            self.course.id, 
            "Course returned should have a different ID to existing course")
    ...
    def test_get_course_for_client_new_courses(self):
        ...
        self.assertNotEqual(course, None, 
                            "Client should be given a new course")
        self.assertNotEqual(
            course.id, 
            self.course.id, 
            "Course returned should have a different ID to existing course")
    ...
    def test_save_tasks(self):
        pre_task_client = Session.objects.filter(
            id=self.hip_session.id).prefetch_related('tasks')[0]
        pre_task = list(pre_task_client.tasks.all())
        self.assertEqual(
            len(pre_task), 0, 
            'Session should have NO tasks before saving tasks')
        post_data = {
        ...
        task_session = Session.objects.filter(
            id=self.hip_session.id).prefetch_related('tasks')[0]
        ...
        for task in tasks:
            self.assertIn(
                task.id, 
                [self.task1.id, self.task2.id], 
                'Task for the session should be one of the defined tasks')
    ...
    def test_save_diagnosis(self):
        pre_diagnoses_client = Client.objects.filter(
            id=self.hip_client.id).prefetch_related('diagnosis')[0]
        pre_diagnoses = list(pre_diagnoses_client.diagnosis.all())
        self.assertEqual(
            len(pre_diagnoses), 0, 
            'Client should have NO diagnoses before saving a diagnosis')
        ...
        diagnoses_client = Client.objects.filter(
            id=self.hip_client.id).prefetch_related('diagnosis')[0]
        diagnoses = list(diagnoses_client.diagnosis.all())
        self.assertEqual(
            len(diagnoses), 2, 
            'Client should have 2 diagnoses after saving the diagnosis')
        for diagnosis in diagnoses:
            self.assertIn(
                diagnosis.id, 
                [self.diagnosis1.id, self.diagnosis2.id], 
                'Task for the session should be one of the defined tasks')
    ...
    def test_save_diagnosis_no_data(self):
        ...
        self.assertEqual(
            len(diagnoses), 0, 
            'Client should have NO diagnoses after saving the diagnosis')
    ...
    def test_convert_observations(self):
        ...
        self.assertEqual(actual_observations,
                          expected_observations, 
                          'Should have converted old observations correctly')
    ...
    def test_create_course_for_client(self):
        ...
        old_course_count = Course.objects.filter(
            client=self.hip_client).count()
        
        create_course_for_client(self.hip_client.id)
        
        new_course = Course.objects.filter(client=self.hip_client).last()
        new_course_count = Course.objects.filter(
            client=self.hip_client).count()
        
        self.assertGreater(new_course.id, 
                           last_course.id, 
                           'New course should have increased the course id')
        self.assertEqual(new_course_count, 
                         old_course_count + 1, 
                         'Should now be one more course for this client')
    ...
    def test_get_scored_courses_for_client(self):
        ...
        self.assertEqual(scored['course'], 
                         self.course.id, 
                         'Should return the first course id')
        self.assertEqual(scored['first_week'], 1, 
                         'First scored week should be week 1')
        self.assertEqual(scored['last_week'], 1, 
                         'Last scored week should be week 1 as well')
        self.assertEqual(
            datetime.strptime(scored['last_date'], 
                              '%a, %d %b %Y').date(), 
            self.hip_session.session_date, 'Dates should be the same')
        self.assertEqual(
            datetime.strptime(scored['first_date'], 
                              '%a, %d %b %Y').date(), 
            self.hip_session.session_date, 'Dates should be the same')
    ...
    def test_get_scored_courses_for_client_multiple_scored_courses(self):
        ...
        SkillScore.objects.create(
            session=next_scored_session, skill=self.basket_weaving, score=2)
        SkillScore.objects.create(
            session=next_scored_session, skill=self.flower_arranging, score=4)
        SkillScore.objects.create(
            session=next_scored_session, skill=self.embroidery, score=1)
        SkillScore.objects.create(
            session=next_scored_session2, skill=self.basket_weaving, score=3)
        SkillScore.objects.create(
            session=next_scored_session2, skill=self.flower_arranging, score=4)
        SkillScore.objects.create(
            session=next_scored_session2, skill=self.embroidery, score=5)

        scored = get_scored_courses_for_client(self.hip_client.id)[0]
        
        self.assertEqual(scored['course'], 
                         self.course.id, 'Should return the first course id')
        self.assertEqual(scored['first_week'], 1, 
                         'First scored week should be week 1')
        self.assertEqual(scored['last_week'], 1,
                          'Last scored week should be week 1 as well')
        ...
        self.assertEqual(
            scored['course'], 
            next_scored_course.id, 'Should return the next course id')
        self.assertEqual(
            scored['first_week'], 1, 
            'First scored week should be week 1')
        self.assertEqual(
            scored['last_week'], 2, 
            'Last scored week should be week 2')
        self.assertEqual(
            datetime.strptime(scored['last_date'], 
                              '%a, %d %b %Y').date(), 
            next_scored_session2.session_date, 'Dates should be the same')
        self.assertEqual(
            datetime.strptime(scored['first_date'], 
                              '%a, %d %b %Y').date(),
             next_scored_session.session_date, 'Dates should be the same')
